			...
			function successCallback(position)
			  {
				 ...
				 $(result).map(function(index, entry) {
					 	test.push({distan:distance(entry.lat,entry.lng,latitude, longitude),
					 		addr1:entry.addr1, phone:entry.phone, cname:entry.campName, x:entry.lat, y:entry.lng, addr2:entry.addr2, image:entry.image, campType:entry.campType});
					 	})
				 ...
				 var txt2 = new Array();
					...
					for(var j=(i-1)*(10)+1;j<10*(i)+1;j++){
						 ...
						 if(test[j-1].campType=="야영장"){
								txt[i] += "<td style='width:320px;padding:5px;margin:5px;'><ul style='margin:0;padding:0;'><li><div style='background-color:#87003A;color:white;text-align:center;font-size: 15px; padding-bottom: 5px; padding-top: 5px; width: 80px;' class='camptype'>"+"야영장"+"</div></li>\r\n";
							}else{
								txt[i] += "<td style='width:320px;padding:5px;margin:5px;'><ul style='margin:0;padding:0;'><li><div style='background-color:#2FED28;color:white;text-align:center;font-size: 15px; padding-bottom: 5px; padding-top: 5px; width: 130px;' class='camptype'>"+"글램핑 / 카라반"+"</div></li>\r\n";
							}
						 txt[i] += "<li><div class='campname'><a style='text-decoration:none;' href=\"javascript:abcd('"+test[j-1].x+"','"+test[j-1].y+"','"+test[j-1].cname+"','"+test[j-1].phone+"','"+test[j-1].addr1+"','"+test[j-1].addr2+"')\" onclick=\"w3_open()\">"+test[j-1].cname+"</a><br></div></li>\r\n";
