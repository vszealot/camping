	...
	<section class="container">
		...
		<div class="search row">
			...
			<c:if test="${!empty logOK}">
				<a role="button" href="qnaWrite.do" class="btn btn-default">글쓰기</a>
			</c:if>
			<c:if test="${empty logOK}">
				<button class="btn btn-default" disabled="disabled">글쓰기</button>
			</c:if>&nbsp;
			<a role="button" href="qnaListSearch.do" class="btn btn-default">글목록</a>
		</div>
		<div class="col-md-offset-3">
			<ul class="pagination">
				<c:if test="${page.prev}">
					<li><a href="qnaListSearch.do${page.makeSearch(page.startPage - 1)}">이전</a></li>
				</c:if>

				<c:forEach begin="${page.startPage}" end="${page.endPage}" var="idx">
					<li <c:out value="${page.page == idx ? 'class=active' : ''}"/>>
					<a href="qnaListSearch.do${page.makeSearch(idx)}">${idx}</a></li>
				</c:forEach>

				<c:if test="${page.next && page.endPage > 0}">
					<li><a href="qnaListSearch.do${page.makeSearch(page.endPage + 1)}">다음</a></li>
				</c:if>
			</ul>
		</div>			
</section>
...
<script>
	...
	$(function() {
		$('#searchBtn').click(
				function() {
					self.location = "qnaListSearch.do"
							+ '${page.makeQuery(1)}'
							+ "&searchType="
	...
	function updateClick(idx1,idx2){
 		$.ajax({
			type:"GET",
			url:"qnaUpdateClick.do?click="+ idx1 +"&postNo=" + idx2,
			success:function(data){
				location.href = "qnaRead.do?postNo="+ idx2 +"&page=${page.page}&perPageNum=${page.perPageNum}&"
