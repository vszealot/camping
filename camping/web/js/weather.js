function realTimeWeather(x, y, z) {
	var today = new Date();
	 ...
	 */
	if (hours == 5 || hours == 8 || hours == 11 || hours == 14 || hours == 17
			|| hours == 20 || hours == 23) {
		if (minutes < 10) {
			hours = hours - 3;
		}
	...
	if (hours == 3 || hours == 6 || hours == 9 || hours == 12 || hours == 15
			|| hours == 18 || hours == 21) {
		hours = hours - 1;
		if (hours == 0) {
			hours = 23;
	...
	if (hours == 4 || hours == 7 || hours == 10 || hours == 13 || hours == 16
			|| hours == 19 || hours == 22) {
		hours = hours - 2;
		if (hours == 1) {
			hours = 23;
	...
	/*
	 * if (hours < 0) { // 자정 이전은 전날로 계산 today.setDate(today.getDate() - 1); day =
	 * today.getDate(); month = today.getMonth() + 1; year =
	...
	$
			.ajax({
				...
				success : function(msg) {
					 ...
					 * console.log(msg.responseText); var re = /(<([^>]+)>)/g;
					 * var t = text.replace(re,""); // HTML 태그 모두 공백으로 대체 text =
					 * '[' + t + ']';
					 */// var weather =
						// JSON.stringify(ForecastGribURL.responseXML)
					// var rain_state =
					// msg.response.body.items.item[1].obsrValue;
					...
					if (month == 01 || month == 03 || month == 05
						...
						if (day1 == 30) {
							...
							var yymmddtomorrow2 = year1 + "" + month1 + ""
									+ day1;
						} else if (day1 == 31) {
							day1 = 2;
							month1 = month1 + 1;
					...
					function first() {
						...
						for (var i = 0; i < 6; i++) {
							...
							table += "<th style=\"text-align:center;\">" + today + "<br>" + time1 + "</th>";
						}
						table += "<th><a href=\"javascript:realTimeWeather("+x+","+y+",2)\">다음 </a></th>";
						table += "</tr><tr>";
						for (var i = 0; i < 6; i++) {
							if (ptyValue[i] != 0) {
								switch (ptyValue[i]) {
								case 1:
									sky = "<img src='/camping/image/heavy_rain.svg' width='110px'/>";
									break;
								case 2:
									sky = "<img src='/camping/image/snowy.svg' width='110px'/>";
									break;
								case 3:
									sky = "<img src='/camping/image/snowy.svg' width='110px'/>";
									break;
								...
								switch (skyValue[i]) {
								case 1:
									sky = "<img src='/camping/image/sunny.svg' width='110px'/>";
									break;
								case 2:
									sky = "<img src='/camping/image/sunny_to_cloudy.svg' width='110px'/>";
									break;
								case 3:
									sky = "<img src='/camping/image/흐림.svg' width='110px'/>";
									break;
								case 4:
									sky = "<img src='/camping/image/overcast.svg' width='110px'/>";
									break;
								}
							}
							table += "<td>" + sky + "</td>";
					...
					function second() {
						var table = "<table style='text-align:center;'><tr><th><a href=\'javascript:realTimeWeather("+x+","+y+",1)\'>이전</a></th>";
						for (var i = 6; i < 12; i++) {
							if (date[i] == yymmddtoday) {
								today = "오늘";
							}
						...
						table += "<th><a href=\'javascript:realTimeWeather("+x+","+y+",3)\'>다음</a></th>";
						table += "</tr><tr>";
						table += "<td> </td>";
						for (var i = 6; i < 12; i++) {
							if (ptyValue[i] != 0) {
					...
					function third() {
						var table = "<table style='text-align:center;'><tr><th><a href=\'javascript:realTimeWeather("+x+","+y+",2)\'>이전</a></th>";
						for (var i = 12; i < 18; i++) {
							if (date[i] == yymmddtoday) {
								today = "오늘";
							}
						...
						table += "<th><a href=\'javascript:realTimeWeather("+x+","+y+",4)\'>다음</a></th>";
						table += "</tr><tr>";
						table += "<td> </td>";
						for (var i = 12; i < 18; i++) {
							if (ptyValue[i] != 0) {
					...
					function fourth() {
						var table = "<table style='text-align:center;'><tr><th><a href=\'javascript:realTimeWeather("+x+","+y+",3)\'>이전</a></th>";
